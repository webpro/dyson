...
const generate = (req, res, next) => {
  ...
  if (!(service.cache && cache[path])) {
    if (!multiRequest.isMultiRequest(path, options)) {
      const isCollection = _.isFunction(service.collection)
          ? service.collection.apply(service, templateArgs)
          : service.collection,
      ...
      promise
        .then(data => {
          return !service.container
            ? data
            : setValues(_.result(service, 'container'), containerArgs.concat([data]), service);
        })
        .then(data => {
    ...
    _.forEach(template, (value, key) => {
      if (template.hasOwnProperty(key)) {
        obj[key] = _.isFunction(value)
          ...
          : _.isPlainObject(value) ? setValues(value, params, obj) : value;
